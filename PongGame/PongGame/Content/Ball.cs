...
namespace PongGame.Content
{
    public class Ball : Sprite
    {
        ...
        public void Move(GameTime gameTime)
        {
            ...
            if (Position.X <= 0 + Rectangle.Width / 2) // Left wall, Player 1
            { Direction.X *= -1; Game1.callPlayer(1); }
            if (Position.X >= graphics.Viewport.Width - Rectangle.Width / 2) // Right Wall, Player 2
            { Direction.X *= -1; Game1.callPlayer(2); }
            if (Position.Y <= 0 + Rectangle.Height / 2) // Upper wall, Player 3
            { Direction.Y *= -1; Game1.callPlayer(3); }
            if (Position.Y >= graphics.Viewport.Height - Rectangle.Height / 2) // Lower wall, Player 4
